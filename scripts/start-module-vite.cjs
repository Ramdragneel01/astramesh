...
if (!modulePathArg || !portArg) {
  console.error("Usage: node scripts/start-module-vite.cjs <modulePath> <port>");
  process.exit(1);
}
...
const child = spawn(`npm run dev -- --port ${port} --host --strictPort`, [], {
  cwd: moduleCwd,
  stdio: "inherit",
  shell: true
});
...
child.on("error", (error) => {
  console.error(`Failed to launch Vite in ${moduleCwd}: ${error.message}`);
  process.exit(1);
});
